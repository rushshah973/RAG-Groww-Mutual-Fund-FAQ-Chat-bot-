# Problem Statement: Mutual Fund FAQ Assistant (Facts-Only Q&A)

## Overview
The objective of this project is to build a facts-only FAQ assistant for mutual fund schemes, using **Groww** as the reference product context. The assistant will answer objective, verifiable queries related to mutual funds by retrieving information exclusively from official public sources, such as AMC (Asset Management Company) websites, AMFI, and SEBI.

The system must strictly avoid providing investment advice, opinions, or recommendations. Every response must include a single, clear source link and adhere to defined constraints around clarity, accuracy, and compliance.

---

## Objective
Design and implement a lightweight **Retrieval-Augmented Generation (RAG)**-based assistant that:
- Answers factual queries about mutual fund schemes.
- Uses a curated corpus of official documents.
- Provides concise, source-backed responses.

---

## Target Users
- **Retail Investors:** Comparing mutual fund schemes and seeking quick factual details.
- **Customer Support & Content Teams:** Handling repetitive mutual fund queries.

---

## System Flow & Architecture

```mermaid
graph TD
    A[User Query] --> B{Advisory or Factual?}
    B -->|Advisory/Opinion/Comparison| C[Refusal Handler]
    B -->|Factual| D{Contains PII?}
    D -->|Yes: PAN/Aadhaar/OTP/Email| E[PII Redaction/Refusal]
    D -->|No| F[Retrieve Context from Curated Corpus]
    F --> G[Generate Facts-Only Response]
    G --> H[Apply Formatting Constraints]
    H --> I[Add Footer & Source Citation]
    I --> J[Display to User]
    
    C --> K[Polite Refusal + Educational Link]
    E --> L[Privacy Warning]
```

---

## Scope of Work

### 1. Corpus Definition
- Compile a structured corpus of top AMCs listed in [groww_amcs.json](file:///Users/rushabh/Mutual%20Fund%20Milestone/groww_amcs.json) (sourced from `https://groww.in/mutual-funds/amc`).
- For each AMC, collect structured facts-only details covering:
  - Scheme factsheets (large cap, mid cap, small cap, debt, and index funds)
  - Expense ratios, exit loads, and minimum SIP amounts
  - ELSS lock-in periods
  - Riskometers and benchmark indexes
  - Fund manager credentials and tenures
  - Customer support contacts and account statement request procedures

### 2. FAQ Assistant Requirements
The assistant must answer facts-only queries, such as:
- Expense ratio of a scheme
- Exit load details
- Minimum SIP amount
- ELSS lock-in period
- Riskometer classification
- Benchmark index
- Fund manager details (e.g., name, experience, credentials, and tenure)
- Process to download statements or capital gains reports

> [!IMPORTANT]
> **Strict Response Constraints:**
> - Each response is limited to a **maximum of 3 sentences**.
> - Each response must include **exactly one citation link**.
> - Each response must include a footer:  
>   `Last updated from sources: <date>`

### 3. Refusal Handling
The assistant must refuse non-factual or advisory queries, such as:
- *"Should I invest in this fund?"*
- *"Which fund is better?"*

**Refusal responses must:**
- Be polite and clearly worded.
- Reinforce the facts-only limitation.
- Provide a relevant educational link (e.g., AMFI or SEBI resource).

### 4. User Interface (Minimal)
The solution should include a simple, clean interface with:
- A welcome message
- Three example questions
- A visible disclaimer:  
  `Facts-only. No investment advice.`

### 5. Automated Database Scheduling
- Implement a background scheduler that runs automatically every day at **09:25 AM**.
- The scheduler must trigger data re-ingestion from the target sources and rebuild the vector store index, ensuring facts, managers, and expense ratios remain continuously up-to-date.

---

## Constraints

### Data and Sources
- Use **only** official public sources (AMC, AMFI, SEBI).
- Do **not** use third-party blogs or aggregator websites.

> [!CAUTION]
> **Privacy and Security (No PII):**
> Do not collect, store, or process:
> - PAN or Aadhaar numbers
> - Account/Folio numbers
> - OTPs
> - Email addresses or phone numbers

### Content Restrictions
- **No investment advice** or recommendations.
- **No performance comparisons** or return calculations.
- For performance-related queries, provide a link to the official factsheet only.

### Transparency
- Responses must be short, factual, and verifiable.
- Every answer must include a source link and last updated date.

---

## Expected Deliverables
- **README Document**
  - Setup instructions
  - Selected AMC and schemes
  - Architecture overview (RAG approach)
  - Known limitations
- **Disclaimer Snippet**
  - `"Facts-only. No investment advice."`
- **Minimal User Interface**
  - The conversational interface.

---

## Success Criteria
- [ ] **Accurate Retrieval:** Factual mutual fund details are correctly sourced.
- [ ] **Strict Adherence:** Refusal of any advisory, speculative, or opinion-based queries.
- [ ] **Consistent Citation:** Inclusion of valid, official source links.
- [ ] **Polite Handling:** Proper refusal and educational redirection for out-of-scope queries.
- [ ] **Clean UI:** Minimal, modern, and user-friendly interface.

---

## Summary
The goal is to build a trustworthy, transparent, and compliant mutual fund FAQ assistant that prioritizes accuracy over intelligence. The system should ensure that users receive only verified, source-backed financial information, without any advisory bias or speculative content.
